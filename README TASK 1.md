...
## 🛡️ Countermeasures Taken for Issues Found

During the EDA process, several issues were identified. Below are the problems encountered and the countermeasures taken:

| **Issue**                                      | **Countermeasure Taken**                                                                 |
|------------------------------------------------|------------------------------------------------------------------------------------------|
| Missing values in `Age` and `Fare` columns     | Imputed missing values with the **median** of each respective column                    |
| Missing values in `Embarked` column            | Filled with the **mode** (most common category)                                         |
| Duplicate rows in the dataset                  | Removed using `df.drop_duplicates()`                                                    |
| Outliers in `Age` and `Fare` columns           | Detected using the **IQR method** and **capped** to limit their influence               |
| Mixed data types or inconsistent values        | Standardized categories and used `df.info()` to verify correct types                    |
| Skewed distributions in Fare and Age           | Visualized with histograms and boxplots to inform further transformation if needed      |
| Unclear correlations between variables         | Generated a **heatmap** to visualize numeric feature relationships                      |

These countermeasures helped ensure the dataset was clean, well-understood, and ready for further analysis or machine learning tasks.
